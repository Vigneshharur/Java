...
const FakeButton = styled('span')<{ type: string }>`
  ...
  border: ${({ type }) => `1px solid ${type === 'disabled' ? colors.gray3 : colors.tertiary2}`};
...
const FileName = styled('span')<{ error?: boolean; disabled?: boolean }>`
  ...
  white-space: nowrap;
  border: ${({ error }) => `1px solid ${error ? colors.yellow4 : colors.purple2}`};
  border-right: none;
...
const CancelInput = styled('span')<{ error?: boolean }>`
  background-color: ${({ error }) => `${error && colors.yellow4}`};
  border: ${({ error }) => `0.1rem solid ${error ? colors.yellow4 : colors.purple2}`};
  border-left: none;
          ...
          <FakeInput>
            ...
            {value.length > 0 && (
              <CancelInput onClick={handleFileCancel} error={!!error}>
                <Icon iconClass="times" color={colors.purple5} />
              </CancelInput>
            )}
            <FakeButton type={disabled ? DISABLED : buttonType}>{buttonText}</FakeButton>
            {error && (
              <FileErrorMessage css="">
                <Icon iconClass="exclamation-triangle" color={colors.yellow4} iconSize="lg" weight="fas" />
                {error}
              </FileErrorMessage>
            )}
          </FakeInput>
